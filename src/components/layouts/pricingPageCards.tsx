...
const PricingCard = ({
  ...
  return (
    <div
      className={cn(
        "relative flex flex-col p-6 mx-auto max-w-lg text-center text-zinc-900 dark:text-zinc-300 rounded-lg border border-zinc-100 shadow dark:border-zinc-800 xl:p-8 h-full w-full",
        highlight && "border-primary/25 dark:border-primary/25 "
      )}
    >
      {highlight && (
        <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
          <Badge className="w-fit uppercase">Most popular</Badge>
        </div>
      )}
      <h3 className="mb-4 text-2xl font-semibold grow-0">{name}</h3>
      <p className="font-light text-muted-foreground sm:text-md dark:text-muted-foreground grow-0">
        {description}
      </p>
      <div className="flex justify-center items-baseline my-8 grow-0">
        <p className="text-5xl font-semibold">
          <span className="mr-0 text-5xl font-semibold">$</span>
          <NumberFlow
